...
function canonicalizePosition(position: string): string[] {
  ...
  if (normalized === "LF" || normalized === "CF" || normalized === "RF") {
    return ["OF"];
  }

  if (normalized === "UTL" || normalized === "UTIL") {
    return ["DH"];
  }
...
export function slotAllowsPosition(slot: string, position: string): boolean {
  ...
  if (normalizedSlot === "MI") return ["2B", "SS", "IF"].includes(normalizedPosition);
  if (normalizedSlot === "CI") return ["1B", "3B", "IF"].includes(normalizedPosition);
  ...
  if (normalizedSlot === "SP") return ["SP", "P"].includes(normalizedPosition);
  if (normalizedSlot === "RP") return ["RP", "P"].includes(normalizedPosition);
